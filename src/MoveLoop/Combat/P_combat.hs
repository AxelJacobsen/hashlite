...
genSlime rng = Enemy{prefix = "a ", eName = "Slime", eMaxHp = 3*rng, eDamage = rng, eArmour = rng-1, eDrops = rng, expDrop = 1}
...
genOrc :: Int -> Enemy
genOrc 0 = genOrc 1
genOrc rng = Enemy{prefix = "an ", eName = "Orc", eMaxHp = 6*rng, eDamage = 3*rng, eArmour = rng*2, eDrops = rng*5, expDrop = 5}
...
enemyDamage :: Enemy -> Player -> StdGen -> (Int, StdGen)
